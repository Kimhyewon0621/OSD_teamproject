...
sys_mmap(void)
{
    ...
    for (uint64 va = ma->addr; va < ma->addr + length; va += PGSIZE) {
        ...
        if (!(flags & MAP_ANONYMOUS) && f != 0) {
            ilock(f->ip);
            readi(f->ip, 0, (uint64)mem, offset + (va - ma->addr), PGSIZE);
            iunlock(f->ip);
        }
...
struct mmap_area*
find_mmap_area(uint64 addr, struct proc *p)
{
    for (int i = 0; i < MAXMMAP; i++) {
        if (mmap_areas[i].p == p &&
            addr >= mmap_areas[i].addr &&
            addr < mmap_areas[i].addr + mmap_areas[i].length) {
            return &mmap_areas[i];
        }
    }
    return 0;
...
void
handle_page_fault(struct mmap_area *ma, uint64 fault_addr)
{
    struct proc *p = myproc();
    uint64 va = PGROUNDDOWN(fault_addr);

    char *mem = kalloc();
    if (mem == 0) { p->killed = 1; return; }
    memset(mem, 0, PGSIZE);

    if (!(ma->flags & MAP_ANONYMOUS) && ma->f != 0) {
        ilock(ma->f->ip);
        readi(ma->f->ip, 0, (uint64)mem,
              ma->offset + (va - ma->addr), PGSIZE);
        iunlock(ma->f->ip);
    }
